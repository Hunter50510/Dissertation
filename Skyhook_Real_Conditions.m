function Skyhook_Real_Conditions
fprintf('\n')
warning('off')
...
% input
inputs = {'Number of rotations per orbit: (For now, always 1)','Length of tether: (Recommended 100-1800)'};
...
dR_dt = alpha_0_R*T/pi;

% Initial conditions
initial_conditions = [r_(1) r_(2) v_(1) v_(2) theta 0 r_P1(1) r_P1(2)...
    r_P2(1) r_P2(2) v_P1(1) v_P1(2) v_P2(1) v_P2(2) dR_dt];
...
figure(9)
plot(time_removed-T,diff_velo_P1_Earth_removed)
hold on
title('Difference between P1 and Earth''s velocity')
xlabel('Time_(_s_)'), ylabel('Velocity (km/s)')
grid on
...
%close figure 9 figure 7 figure 6 figure 5 figure 4 figure 3 figure 2 figure 8

min_delta_v = min(diff_velo_P1_Earth_removed);
fprintf('The minimum difference between the Earth and P1 = %gkm/s\n\n',min_delta_v)
RPM = rot_orbit/(T/60);
g_force = (RPM/1000)^2*(sat_length*1000^2)*1.118;

fprintf('g-force of P1 = %gg\n',g_force)
...
fprintf('%g\n',mean(P2_dist))
    ...
    T_R = 2*pi*sqrt(sqrt(x^2+y^2)/mu);
    rho_R = 5.77e-7*(100/sat_length); %http://www.braeunig.us/space/atmos.htm   https://www.ukssdc.ac.uk/cgi-bin/wdcc1/secure/msis_models.pl
    v_R = sqrt(u_P1.^2+v_P1.^2);

    alpha_0_R = 0.5*rho_R*v_R^2*cd_R*(A/m);
    dU(15) = alpha_0_R*T_R/pi;
